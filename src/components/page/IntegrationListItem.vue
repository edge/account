<template>
  ...
  >
    ...
    <div class="cdnList__field domain overflow-hidden">
      ...
      <span class="text-m" :title="domain">{{ domain }}</span>
    </div>
    ...
    <!-- updated -->
    <div class="cdnList__field updated overflow-hidden">
      <span class="cdnList__header">Last Updated</span>
      <span class="text-m" :title="updated">{{ updated }}</span>
    </div>
...
export default {
  ...
  computed: {
    ...
    updated() {
      const updated = moment(this.integration.updated).fromNow()
      return updated === 'a few seconds ago' ? 'Just now' : updated
    },
    isActive() {
  ...
  methods: {
    goToIntegration() {
      this.$router.push(`/page/${this.integration._key}`)
    }
  }
}
</script>
